...
import sbt.Keys._
import sbt.Using.{fileInputStream, zipInputStream, fileOutputStream}
...
import scala.xml.{Node => XmlNode, XML, NodeSeq, Elem}
...
object MergeWebappPlugin extends AutoPlugin {
  ...
  val DergeWebappConfig = config("merge-webapp")
  ...
  lazy val mergeWebappSettings: Seq[Setting[_]] = inConfig(DergeWebappConfig)(Seq[Setting[_]](
    ...
    merge := {
      ...
      val currentMappingSettings = mapping.map { case ((groupNameMapp, artifactNameMapp), folderMapping) =>
        ...
        val lib = managedcp.filter(_.get(Keys.moduleID.key).exists(x =>
          x.name == fullLibDep.name && x.organization == fullLibDep.organization && x.revision == fullLibDep.revision
        )).head
    ...
    resourceGenerators in Compile <+= merge in DergeWebappConfig
  )
}
...
case class MappingDirectories(organization: String, artifact: String, revision: String, mapping: Seq[MappingPair]) {
  ...
  def toXML: Elem = <mapping organization={organization} artifact={artifact} revision={revision}><content>{mapping.map(_.toXML)}</content></mapping>
  ...
  def getLibFile(managedClasspath: Seq[Attributed[File]]): File = {
    val lib = managedClasspath.filter(_.get(Keys.moduleID.key).exists(x => (x.name == artifact) && (x.organization == organization) && (x.revision == revision))).head
    lib.data
  }

  def unpackMappings(managedClasspath: Seq[Attributed[File]], tempDir: File, srcDir: File)(implicit logger: Logger): Unit = {
    val sourceFile = getLibFile(managedClasspath)
    logger.info(newLine)
